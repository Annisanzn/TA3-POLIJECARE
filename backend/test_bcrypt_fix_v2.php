...
echo "=== Testing Bcrypt Algorithm Check (Direct DB Update) ===\n";
...
if ($user) {
    // BYPASS ELOQUENT CASTING by using DB facade
    DB::table('users')->where('id', $user->id)->update(['password' => 'md5hashedpassword123']);
    echo "User password set to non-bcrypt string directly in DB.\n";
} else {
    echo "User not found.\n";
    exit(1);
}
...
if (str_contains($response->getContent(), 'This password does not use the Bcrypt algorithm.')) {
    echo "✅ Success: Correct error message returned for non-bcrypt password.\n";
} else {
